...
namespace GPUTutorial
{
   ...
   StatusCode LinearTransformCUDAAlg::execute(const EventContext &ctx) const
   {
      ...
      // Run the kernel. Re-using the "n" variable from above.
      const std::size_t blockSize = 256;
      const std::size_t numBlocks = (n + blockSize - 1) / blockSize; // FIX
      ...
      // FIX Free the device memory.
      ATH_CUDA_CHECK(cudaFree(inputDevice));
      ATH_CUDA_CHECK(cudaFree(outputDevice));
      // FIX
